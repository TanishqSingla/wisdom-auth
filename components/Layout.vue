<script>
export default {};
</script>

<template>
	<Header />
	<main class="flex h-screen">
		<section class="hidden lg:flex flex-col bg-[#404555] w-[40%]">
			<div class="h-[60%] grid place-items-center">
				<svg
					width="217"
					height="153"
					viewBox="0 0 217 153"
					fill="none"
					xmlns="http://www.w3.org/2000/svg"
				>
					<path
						d="M119.868 126.696H39.3998L0 0L80.9026 0L119.868 126.696Z"
						fill="#F1C12B"
					/>
					<g opacity="0.5">
						<path
							opacity="0.5"
							d="M217 131.182C217 135.497 215.725 139.715 213.338 143.303C210.951 146.891 207.558 149.688 203.589 151.339C199.619 152.99 195.251 153.422 191.037 152.58C186.823 151.739 182.952 149.661 179.914 146.609C176.876 143.558 174.806 139.671 173.968 135.438C173.13 131.206 173.56 126.819 175.204 122.833C176.849 118.846 179.633 115.439 183.206 113.041C186.778 110.644 190.979 109.364 195.275 109.364C201.037 109.364 206.563 111.663 210.637 115.755C214.711 119.846 217 125.396 217 131.182Z"
							fill="white"
						/>
					</g>
					<path
						d="M75.9609 153H155.55L194.512 26.3036L114.502 26.3036L75.9609 153Z"
						fill="white"
					/>
				</svg>
			</div>
			<div class="h-[40%] flex">
				<img src="../assets/images/POSE_02.png" class="self-end" />
				<div class="w-full h-full flex flex-col self-center gap-4 justify-center">
					<p class="text-white text-2xl font-extrabold">Welcome back!</p>
					<p class="text-white mb-8 font-normal mr-6 max-w-xs">
						Sign In to find opportunities that match your interests. We have
						both part-time and full-time roles that can be done online and
						in-person.
					</p>
					<svg
            class="mb-8"
						width="27"
						height="3"
						viewBox="0 0 27 3"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
					>
						<rect width="15.0615" height="3" rx="1.5" fill="white" />
						<rect
							opacity="0.5"
							x="17.8857"
							width="2.82402"
							height="3"
							rx="1.41201"
							fill="white"
						/>
						<rect
							opacity="0.5"
							x="23.5332"
							width="2.82402"
							height="3"
							rx="1.41201"
							fill="white"
						/>
					</svg>
					<p class="text-xs max-w-xs">Please contact us at <strong>+91-9380644532</strong> if you need any assistance.</p>
				</div>
			</div>
		</section>
		<slot name="children"></slot>
	</main>
</template>

<style>
main {
  color: white;
	font-family: "Poppins", sans-serif;
}
</style>
